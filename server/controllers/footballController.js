...
class FootballController{
    ...
    static getAllStandings(req,res,next){
        let league_id = req.params.league_id
        axios.get(`https://api-football-v1.p.rapidapi.com/v2/leagueTable/${league_id}`, {
            headers: {"X-RapidAPI-Host": process.env.RAPID_HOST, "X-RapidAPI-Key": process.env.RAPID_KEY}
        })
            .then(results =>{
                // console.log(results)
                let data = results.data.api.standings[0]
                res.status(200).json(data)
            })
    }
    ...
    static getTopScorer(req,res,next){
        let league_id = req.params.league_id
        axios.get(`https://api-football-v1.p.rapidapi.com/v2/topscorers/${league_id}`, {
            headers: {"X-RapidAPI-Host": process.env.RAPID_HOST, "X-RapidAPI-Key": process.env.RAPID_KEY}
        })
            .then(results => {
                let data = results.data.api.topscorers
                res.status(200).json(data)
            })
            .catch(err => {
                res.status(500).json(err)
